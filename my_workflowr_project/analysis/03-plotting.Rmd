...
```{r}
qplot(data = iris, x = Sepal.Length, y = Sepal.Width, colour = Species, geom = "point")
```
...
p1 <- ggplot(data = iris) +
  ...
  facet_wrap(. ~ Species, labeller = labeller(Species = c("setosa" = "Setosa", 
                                                          ...
                                                          "virginica" = "Virginica"))) +
  scale_color_manual(values = colour_vec, name = "Iris specis", labels = c("Setosa", "Versicolor", "Virginica")) +
  scale_fill_manual(values = colour_vec, name = "Iris specis", labels = c("Setosa", "Versicolor", "Virginica")) +
  xlab("Sepal Length") +
  ylab("Sepal Width") +
...
```{r}
source("/Users/michelle/Documents/Github/learn-git/my_workflowr_project/code/helper.R")
...
Plot the distribution of sepal length across different species

```{r}
set.seed(12345)
p3 <- ggplot(data = iris) +
  geom_boxplot(mapping = aes(x = Species, y = Sepal.Length, fill = Species), outlier.colour = NA) +
...
Calculate mean, sd, se and ci of the 4 different features per species
...
rows: all features, cols: all samples
